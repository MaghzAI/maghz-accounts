...
import { DollarSign, Loader2, Printer, Download, RefreshCw } from "lucide-react";
...
export default function InventoryValuationPage() {
  ...
  const [filteredData, setFilteredData] = useState<ValuationItem[]>([]);
  ...
  async function fetchData() {
    ...
    try {
      ...
      const response = await fetch(`/api/reports/inventory/valuation?${params}`);
      if (response.ok) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Filter inventory valuation</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <tbody>
                  {filteredData.map((item) => (
                    <tr key={item.id} className="border-b hover:bg-muted/50">
                      ...
                      <td className="p-3 text-right text-sm">{item.quantity} {item.unit}</td>
