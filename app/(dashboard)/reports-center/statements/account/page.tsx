...
import { FileText, Loader2, Printer, Download, RefreshCw } from "lucide-react";
...
export default function AccountStatementPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const response = await fetch(`/api/reports/statements/account?${params}`);
      if (response.ok) {
        const reportData = await response.json();
        setSelectedAccount(reportData.account);
        setData(reportData.transactions);
        setSummary(reportData.summary);
      } else {
        toast({
          title: "Error",
          description: "Failed to load account statement",
          variant: "destructive",
        });
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={handlePrint} disabled={!selectedAccount}>
          ...
          <CardContent className="pt-6">
            <div className="grid grid-cols-4 gap-4">
              <div>
                ...
                <p className="font-mono font-bold">{selectedAccount.code}</p>
              </div>
              <div>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Debit entries</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className={`text-2xl font-bold ${summary.closingBalance >= 0 ? "text-green-600" : "text-red-600"}`}>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="flex gap-2 mt-4">
            <Button onClick={fetchData} disabled={!filters.accountId}>
              Generate Statement
            </Button>
      ...
      {selectedAccount && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Showing {data.length} transactions for {selectedAccount.name}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
                ...
                <table className="w-full">
                  ...
                  <tfoot>
                    <tr className="border-t-2 bg-muted/50 font-semibold">
                      ...
                      <td className="p-3 text-right text-sm text-blue-600">{formatCurrency(summary.totalDebits)}</td>
                      <td className="p-3 text-right text-sm text-orange-600">{formatCurrency(summary.totalCredits)}</td>
